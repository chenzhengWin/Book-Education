<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="bookList.aspx.cs" Inherits="TuShuJieYueYi_Web.Admin.Books.bookList" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
  <title>图书管理列表</title>
<link rel="stylesheet" href="../css/style.css" />
    <link rel="stylesheet" href="../css/common.css" />
    <link rel="stylesheet" href="../css/main.css" />
    <script type="text/javascript" src="../js/jquery.min.js"></script>
    <script type="text/javascript" src="../js/colResizable-1.3.min.js"></script>
    <script type="text/javascript" src="../js/common.js"></script>
    <style>
        .right {
            display:inline-block;
            margin-left:100px;
            /*position:fixed;
            right:100px;*/
        }
    </style>
    <script type="text/javascript">
        $(function () {
            $(".list_table").colResizable({
                liveDrag: true,
                gripInnerHtml: "<div class='grip'></div>",
                draggingClass: "dragging",
                minWidth: 30
            });

        });
    </script>
</head>
<body>
    <form id="form1" runat="server">
        <div id="button" class="mt10">
            <input type="button" name="button" class="btn btn82 btn_add" value="新增" runat="server" onclick="javascript: window.location.href = 'bookAdd.aspx'" />
            <div class="right">
                <label>请输入关键字：</label><input type="text" class="input-text" name="key" id="txtkey" placeholder=" 书名" value="" runat="server" />
                <input type="button" name="button" class="btn btn82 btn_search" value="查询" runat="server" onserverclick="Query_ServerClick"/>    
            </div>
        </div>
        <div id="table" class="mt10" style="height:800px; overflow:auto;">
            <div class="box span10 oh" style="overflow:scroll;width:100%;height:800px">
                <table width="100%" border="0" cellpadding="0" cellspacing="0" class="list_table" style="text-align: center">
                    <tr>
                        <th width="5%">序号
                        </th>
                        <th width="10%">书名
                        </th>
                        <th width="10%">作者
                        </th>
                        <th width="10%">出版社
                        </th>
                        <th width="10%">出版年
                        </th>
                        <th width="10%">定价
                        </th>
                        <th width="10%">装帧
                        </th>
                        <th width="10%">ISBN
                        </th>
                        <th width="15%">图片地址</th>
                        <th width="10%">操作</th>
                    </tr>
                    <asp:Repeater ID="repList" runat="server" OnItemCommand="repList_ItemCommand"><%-- OnItemCommand="repListItemCommand"--%>
                        <ItemTemplate>
                            <tr class="tr">
                                <td>
                                    <%# Container.ItemIndex+1 %>
                                </td>
                                <td>
                                    <%#Eval("bookname") %>
                                </td>
                                <td>
                                    <%#Eval("author") %>
                                </td>
                                <td>
                                    <%#Eval("publishing_house") %>
                                </td>
                                <td>
                                    <%#Eval("publishing_year") %>
                                    <%--数据库为空时会出错--%>
                                    <%--<%#Convert.ToDateTime(Eval("publishing_year")).ToString("yyyy-MM--dd") %>--%>
                                </td>
                                <td>
                                    <%#Eval("price") %>元
                                </td>
                                <td>
                                    <%#Eval("decorate") %>
                                </td>
                                <td>
                                    <%#Eval("number") %>
                                </td>
                                <td>
                                    <img src="<%#Eval("img") %>" />
                                </td>
                                <td class="td_center">
                                    <asp:LinkButton runat="server" CommandName="Edit" CommandArgument='<%#Eval("Id") %>'
                                        Text="修改"></asp:LinkButton>
                                    <asp:LinkButton runat="server" CommandName="Del" CommandArgument='<%#Eval("Id") %>'
                                        Text="删除" OnClientClick="return confirm('确认删除吗?')"></asp:LinkButton>
                                </td>
                            </tr>
                        </ItemTemplate>
                    </asp:Repeater>
                </table>
                <div class="page mt10">
                    <div class="pagination">
                       <%-- <ul>
                        <li class="first-child"><a href="#">首页</a></li>
                        <li class="disabled"><span>上一页</span></li>
                        <li class="active"><span>1</span></li>
                        <li><a href="#">2</a></li>
                        <li><a href="#">下一页</a></li>
                        <li class="last-child"><a href="#">末页</a></li>
                    </ul>--%>
                       <%--分页，需要进行外部dll文件引用才能够使用--%>
                         <webdiyer:AspNetPager ID="pager" runat="server" OnPageChanged="pager_PageChanged"
                            NumericButtonCount="2" PageSize="2" AlwaysShow="True" ShowBoxThreshold="2" FirstPageText="首页"
                            LastPageText="末页" PrevPageText="上一页" NextPageText="下一页" NumericButtonTextFormatString="{0}"
                            CustomInfoHTML="当前页:%CurrentPageIndex%&nbsp;总页数:%PageCount%&nbsp;总记录数:%RecordCount%&nbsp;&nbsp;&nbsp;&nbsp;"
                            CustomInfoTextAlign="Right" ShowCustomInfoSection="Left">
                        </webdiyer:AspNetPager>
                      
                    </div>  
                     
                </div>
            </div>
        </div>
    </form>
</body>
</html>
